...
我们已经看到，一般来说，加密**不能**解决**消息完整性**的问题。
相反，需要一个**额外的机制**，使通信双方能够知道一条消息是否被篡改。
完成这项任务的正确工具是**消息认证码 (MAC)**。

消息认证码的目标是**防止攻击者修改一方发送给另一方的消息，或注入一条新消息**，而接收方没有检测到该消息并非源自预期的发送方。
与加密的情况一样，这只有在通信双方共享某个攻击者不知道的**秘密**时才可能（否则攻击者无法阻止冒充发送消息的一方）。
在这里，我们将继续考虑**私钥设置**，即双方共享相同的秘密密钥。

**消息认证码的语法**

在正式定义消息认证码的安全性之前，我们首先定义 MAC 是什么以及如何使用它。
希望以**认证**方式通信的两个用户首先**生成并共享一个秘密密钥 $k$**。
当一方想要向另一方发送消息 $m$ 时，她根据消息和共享密钥计算一个 **MAC 标签**（或简称为**标签**）$t$，并将消息 $m$ 和标签 $t$ 发送给另一方。
标签是使用**标签生成算法**（用 $Mac$ 表示）计算的；因此，用我们已经说过的重新措辞，消息 $m$ 的发送方计算 $t \leftarrow Mac_k(m)$ 并将 $\langle m, t \rangle$ 传输给接收方。
接收方**验证** $t$ 是否是消息 $m$ 的**有效标签**（相对于共享密钥）**或不是**。
这是通过运行一个**验证算法** $Vrfy$ 来完成的，该算法以**共享密钥**以及消息 $m$ 和标签 $t$ 作为输入，并指示给定的标签是否有效。
形式上：

**定义 4.1** **消息认证码**（或 **MAC**）由三个**概率多项式时间**算法 $(Gen, Mac, Vrfy)$ 组成，满足以下条件：
1. **密钥生成算法 $Gen$** 以安全参数 $1^n$ 为输入，并输出一个密钥 $k$，其中 $|k| \ge n$。
2. **标签生成算法 $Mac$** 以密钥 $k$ 和消息 $m \in \{0, 1\}^*$ 为输入，并输出一个标签 $t$。
由于此算法可能是随机化的，我们将其记为 $t \leftarrow Mac_k(m)$。
3. **确定性验证算法 $Vrfy$** 以密钥 $k$、消息 $m$ 和标签 $t$ 为输入。
它输出一个比特 $b$，其中 $b=1$ 表示有效，$b=0$ 表示无效。
我们将其记为 $b := Vrfy_k(m, t)$。
**要求** 对于每一个 $n$，由 $Gen(1^n)$ 输出的每一个密钥 $k$，以及每一个 $m \in \{0, 1\}^*$，都有 $Vrfy_k(m, Mac_k(m)) = 1$ 成立。

如果存在一个函数 $l$ 使得对于由 $Gen(1^n)$ 输出的每个 $k$，算法 $Mac_k$ 仅对长度为 $l(n)$ 的消息 $m \in \{0, 1\}^{l(n)}$ 有定义，则我们称该方案是一个用于加密长度为 $l(n)$ 的消息的**定长 MAC**。

与私钥加密一样， $Gen(1^n)$ 几乎总是简单地选择一个均匀的 $k \in \{0, 1\}^n$ 作为密钥，在这种情况下我们省略 $Gen$。

**规范验证。**
对于**确定性**消息认证码（即，其中 $Mac$ 是一个确定性算法），执行验证的**规范方式**是简单地**重新计算标签**并检查是否相等。
换句话说， $Vrfy_k(m, t)$ 首先计算 $\tilde{t} := Mac_k(m)$，然后当且仅当 $\tilde{t} = t$ 时输出1。
然而，即使对于确定性 MAC，定义一个单独的 $Vrfy$ 算法也是有用的，以便明确区分认证消息的**语义**与验证其**真实性**。

**消息认证码的安全性**

我们现在定义消息认证码的**默认安全概念**。
直观的想法是，**任何高效攻击者**都不应该能够在**任何“新”消息**上生成一个有效标签，该消息是通信方以前从未发送（和认证）过的。

与任何安全定义一样，为了形式化这一概念，我们必须定义攻击者的能力以及什么应被视为**“破解”**。
像往常一样，我们只考虑**概率多项式时间攻击者**
...（直到页尾）
...
**4.1** 假设 $\Pi = (Gen, Mac, Vrfy)$ 是一个**安全 MAC**，并且对于 $k \in \{0, 1\}^n$，标签生成算法 $Mac_k$ **总是**输出长度为 $t(n)$ 的标签。
证明 $t$ **必须是超对数**的，或等价地，如果 $t(n) = O(\log n)$，则 $\Pi$ **不能是安全 MAC**。

**提示：** 考虑随机猜测有效标签的概率。

**4.2** 考虑**扩展**的消息认证安全定义，其中攻击者额外被赋予了**Mac 预言机**和 **Vrfy 预言机**的访问权限。
(a) 为这种情况提供一个**形式化的安全定义**。
(b) 假设 $\Pi$ 是一个使用**规范验证**的确定性 MAC，满足**定义 4.2**。
证明 $\Pi$ 也满足您在第 (a) 部分中的定义。

**4.3** 假设**安全 MAC** 存在。
给出一个 MAC 的构造，它相对于**定义 4.2** 是**安全**的，但当攻击者额外被赋予**Vrfy 预言机**的访问权限时（参见前一个练习）是**不安全**的。

**4.4** 证明**命题 4.4**。

**4.5** 假设**安全 MAC** 存在。
证明存在一个 MAC，它（根据**定义 4.2**）是**安全**的，但（根据**定义 4.3**）**不是强安全**的。

**4.6** 考虑以下用于加密长度为 $l(n) = 2n - 2$ 的消息的 MAC，使用了伪随机函数 $F$：
输入消息 $m_0m_1$（其中 $|m_0| = |m_1| = n - 1$）和密钥 $k \in \{0, 1\}^n$，算法 $Mac$ 输出 $t = F_k(\langle 0, m_0 \rangle) \parallel F_k(\langle 1, m_1 \rangle)$。
算法 $Vrfy$ 以自然方式定义。
$(Gen, Mac, Vrfy)$ 是**安全**的吗？
证明您的答案。

**4.7** 令 $F$ 是一个伪随机函数。
证明以下每个 MAC 都是**不安全**的，即使用于认证**定长消息**：
（在每种情况下 $Gen$ 输出均匀 $k \in \{0, 1\}^n$。
令 $\bar{i}$ 表示整数 $i$ 的 $n/2$-比特编码。）
(a) 认证消息 $m = m_1, \ldots, m_l$，其中 $m_i \in \{0, 1\}^n$，计算 $t := F_k(m_1) \oplus \cdots \oplus F_k(m_l)$。
(b) 认证消息 $m = m_1, \ldots, m_l$，其中 $m_i \in \{0, 1\}^{n/2}$，计算 $t := F_k(\langle 1, m_1 \rangle) \oplus \cdots \oplus F_k(\langle \bar{l}, m_l \rangle)$。
(c) 认证消息 $m = m_1, \ldots, m_l$，其中 $m_i \in \{0, 1\}^{n/2}$，选择均匀 $r \leftarrow \{0, 1\}^n$，计算
$$
t := F_k(r) \oplus F_k(\langle \bar{1}, m_1 \rangle) \oplus \cdots \oplus F_k(\langle \bar{l}, m_l \rangle),
$$
并令标签为 $\langle r, t \rangle$。

**4.8** 令 $F$ 是一个伪随机函数。
证明以下用于加密长度为 $2n$ 的消息的 MAC 是**不安全**的：
$Gen$ 输出均匀 $k \in \{0, 1\}^n$。
认证消息 $m_1m_2$，其中 $|m_1| = |m_2| = n$，计算标签 $F_k(m_1) \parallel F_k(F_k(m_2))$。

**4.9** 给定任何**确定性 MAC** $(Mac, Vrfy)$，我们可以将 $Mac$ 视为一个密钥函数。
在**构造 4.5** 和 **4.11** 中， $Mac$ 是一个伪随机函数。
给出一个**安全**、**确定性 MAC** 的构造，其中 $Mac$ **不是**一个伪随机函数。

**4.10** **构造 4.5** 在使用**弱伪随机函数**（参见练习3.26）实例化时**必然是安全**的吗？
解释。

**4.11** 证明**构造 4.7** 是**安全**的，即使攻击者额外被赋予了 **Vrfy 预言机**的访问权限（参见练习4.2）。

**4.12** 证明**构造 4.7** 如果按以下方式更改是**安全**的：
设置 $t_i := F_k(\langle r, b, i, m_i \rangle)$，其中 $b$ 是一个**单个比特**，使得在**所有块**中 $b = 0$，而在**最后一个块**中 $b = 1$。
（为简单起见，假设所有被认证的消息的长度**始终是** $n/2 - 1$ 的整数倍。）
这种修改的优势是什么？

**4.13** 我们探讨当**基本 CBC-MAC 构造**用于**不同长度**的消息时会发生什么。
(a) 假设发送方和接收方**事先**未约定消息长度（因此 $Vrfy_k(m, t) = 1$ 当且仅当 $t = Mac_k(m)$，**无论** $m$ 的长度），但发送方**只**小心地认证长度为 $2n$ 的消息。
证明攻击者可以伪造一个长度为 $4n$ 的消息的有效标签。
(b) 假设接收方**只接受**3块消息（因此 $Vrfy_k(m, t) = 1$ 当且仅当 $m$ 的长度为 $3n$ 且 $t = Mac_k(m)$），但发送方认证**任何** $n$ 的倍数长度的消息。
证明攻击者可以伪造一个**新**消息的有效标签。

**4.14** 证明**基本 CBC-MAC** 的以下修改**不产生**一个**安全 MAC**（即使对于定长消息）：
(a) $Mac$ 输出**所有块** $t_1, \ldots, t_l$，而不是仅输出 $t_l$。
（验证只检查 $t_l$ 是否正确。）
(b) 每次认证消息时使用一个**随机初始块**。
也就是说，选择均匀 $t_0 \in \{0, 1\}^n$，在“消息” $t_0, m_1, \ldots, m_l$ 上运行基本 CBC-MAC，并输出标签 $\langle t_0, t_l \rangle$。
验证以自然方式完成。

**4.15** 证明在应用**基本 CBC-MAC** 之前将**消息长度**附加到**消息末尾**，**不**会产生一个用于**任意长度消息**的**安全 MAC**。

**4.16** 证明第4.4.2节中描述的**任意长度消息的编码是前缀自由**的。

**4.17** 考虑以下处理长度**小于** $n \cdot 2^n$ 的消息的编码：
我们通过首先**附加足够多的0**，使得得到的字符串 $\hat{m}$ 的长度是 $n$ 的**非零倍数**来编码字符串 $m \in \{0, 1\}^*$。
然后，我们将 $\hat{m}$ 中的**块数**（等价地，将整数 $|\hat{m}|/n$）编码为 $n$ 比特字符串作为**前缀**。
证明这种编码**不是前缀自由**的。
---