...
一个流密码在基本意义上是安全的，如果**没有 $IV$**，并且对于任何多项式 $l$ 来说，上面构造的函数 $G_l$ 是一个扩展因子为 $l$ 的伪随机生成器。
...
只要某个基础问题是困难的，一个密码学构造就是安全的。这个**证明**通常是通过呈现一个**显式规约**来进行的，该规约展示了如何将任何成功“破解”该构造的有效攻击者 $A$ 转换为一个解决被假定为困难的问题的高效算法 $A'$。
