...
1.  **情况 1：** $L \ne L'$。在这种情况下，$H^s(x)$ 的计算的最后一步是 $z_{B+1} := h^s(z_B \| L)$，而 $H^s(x')$ 的计算的最后一步是 $z'_{B'+1} := h^s(z'_{B'} \| L')$。由于 $H^s(x) = H^s(x')$，因此 $h^s(z_B \| L) = h^s(z'_{B'} \| L')$。然而，$L \ne L'$，因此 $z_B \| L$ 和 $z'_{B'} \| L'$ 是在 $h^s$ 下发生碰撞的两个不同字符串。
2.  **情况 2：** $L = L'$。这意味着 $B = B'$。设 $z_0, \ldots, z_{B+1}$ 是在计算 $H^s(x)$ 期间定义的值，设 $I_i \stackrel{\text{def}}{=} z_{i-1} \| x_i$ 是 $h^s$ 的第 $i$ 个输入，并设 $I_{B+2} \stackrel{\text{def}}{=} z_{B+1}$。类似地，对于 $x'$ 定义 $I'_1, \ldots, I'_{B+2}$。设 $N$ 是使得 $I_N \ne I'_N$ 的最大索引。由于 $|x| = |x'|$ 但 $x \ne x'$，因此存在一个 $i$ 使得 $x_i \ne x'_i$，因此这样的 $N$ 必然存在。因为
    $$I_{B+2} = z_{B+1} = H^s(x) = H^s(x') = z'_{B+1} = I'_{B+2},$$
    我们有 $N \le B+1$。根据 $N$ 的最大性，我们有 $I_{N+1} = I'_{N+1}$，特别是 $z_N = z'_N$。但这仅仅意味着 $I_N, I'_N$ 是 $h^s$ 中的**一个**碰撞。
...
$$\tilde{\text{Mac}_{k_{\text{out}}}}(y) \stackrel{\text{def}}{=} h^s(k_{\text{out}} \| \hat{y})\quad(5.2)$$
是安全固定长度 MAC，那么 HMAC 可以被视为哈希-认证码方法的实例化
$$\text{HMAC}_{s, k}(m) = \tilde{\text{Mac}_{k_{\text{out}}}}(\tilde{H}^s(m)) \quad(5.3)$$
（其中 $k_{\text{out}} = h^s(IV \| (k \oplus \text{opad}))$）。由于压缩函数 $h$ 的一般设计方式（见 6.3.1 节），假设 $\tilde{Mac}$ 是安全固定长度 MAC 是合理的。

**ipad 和 opad 的作用。** 综上所述，人们可能会疑惑为什么有必要将 $k$ 包含在“内部”计算 $H^s((k \oplus \text{ipad}) \| m)$ 中。
（特别是，对于“Hash-and-MAC”方法的安全性而言，我们要求第一步具备抗碰撞性，而这一步并不需要任何密钥。）
原因在于，这使得 HMAC 的安全性可以基于一个潜在更弱的假设，即 $(\text{Gen}_H, H)$ 是**弱抗碰撞**的；弱抗碰撞性通过以下实验定义：使用 $\text{Gen}_H$ 生成一个密钥 $s$，并选取一个均匀分布的秘密值 $k_{in} \in \{0, 1\}^n$。
然后，允许对手与一个“哈希预言机”进行交互，该预言机针对查询 $m$ 返回 $H^s_{k_{in}}(m)$，其中 $H^s_{k_{in}}$ 指的是对 $h^s$ 应用 Merkle-Damgård 变换来计算 $H^s$，但使用秘密值 $k_{in}$ 作为初始向量（IV）。
（请再次参考图 5.2）
如果对手能够输出不同的值 $m, m'$ 使得 $H^s_{k_{in}}(m) = H^s_{k_{in}}(m')$，则对手攻击成功；如果对于每一个概率多项式时间的算法 $\mathcal{A}$，在该实验中成功的概率仅为可忽略，我们就称 $(\text{Gen}_H, H)$ 是**弱抗碰撞**的。
如果 $(\text{Gen}_H, H)$ 是抗碰撞的，那么它显然也是弱抗碰撞的；然而，后者（弱抗碰撞性）是一个更弱的条件，在潜在层面上更容易被满足。
这是一个稳健的安全工程设计的绝佳范例。
当人们发现哈希函数 MD5（见 6.3.2 节）并不具备抗碰撞性时，这种防御性的设计策略体现了其价值。
针对 MD5 的碰撞发现攻击并没有破坏其“弱抗碰撞性”，因此即便 MD5 本身被攻破了，HMAC-MD5 依然未被攻破。
这给了开发者时间在 HMAC 实现中替换 MD5，而无需担心迫在眉睫的攻击。
（尽管如此，既然 MD5 的弱点已众所周知，就不应再使用 HMAC-MD5 了。）

上述讨论表明，在外部和内部计算中应该使用**独立密钥**。出于效率原因，HMAC 使用单个密钥 $k$，但该密钥与 $\text{ipad}$ 和 $\text{opad}$ 结合使用以导出另外两个密钥。定义
$$G^s(k) \stackrel{\text{def}}{=} h^s(IV \| (k \oplus \text{opad})) \| h^s(IV \| (k \oplus \text{ipad})) = k_{\text{out}} \| k_{\text{in}} \quad(5.4)$$
